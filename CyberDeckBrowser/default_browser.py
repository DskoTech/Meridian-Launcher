"""
default_browser.py — register/unregister CyberDeckBrowser as a handler for
...
so this registers CyberDeck as an available, fully-capable browser choice
...
PROGID = "CyberDeckBrowserHTML"
APPREG = "CyberDeckBrowser"
...
def register(exe_path):
    """Register CyberDeck as a capable browser and associate the http/https/
    ...
    try:
        ...
        _set(H, r"Software\Classes\%s" % PROGID, "CyberDeck Browser Document")
        _set(H, r"Software\Classes\%s\DefaultIcon" % PROGID, '"%s",0' % exe_path)
        _set(H, r"Software\Classes\%s\shell\open\command" % PROGID, cmd)

        # Capabilities block so CyberDeck shows up in Settings > Default apps
        capbase = r"Software\CyberDeckBrowser\Capabilities"
        _set(H, capbase, "CyberDeck Browser", "ApplicationName")
...
def unregister():
    ...
    try:
        ...
        _delete_tree(H, r"Software\CyberDeckBrowser")
        try:
